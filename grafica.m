function handleFigura = grafica(simulacion, solucion, tipoDeGrafico)
...
%	coordenadasBorde = geometria.Borde.coordenadasXY;
%	plot(coordenadasBorde(:,1), coordenadasBorde(:,2))
%	hold on

%	% Grafica la solucion Eta

%	deltaX = geometria.deltaX;
%	deltaY = geometria.deltaY;
%	frac = 0.5;
%	nuevoDeltaX = frac*deltaX;
%	nuevoDeltaY = frac*deltaY;
%	% [valorInterpolado coordenadasInterpoladas] = Interpolador(simulacion, solucion, tipoDeGrafico, nuevoDeltaX, nuevoDeltaY);
%	coordenadasInterpoladas = geometria.Malla.coordenadasEta;
%	valorInterpolado = solucion(1:316);	
%	[valorParaPatch coordenadasX coordenadasY] = puntosParaPatch(valorInterpolado, coordenadasInterpoladas, deltaX, deltaY);
